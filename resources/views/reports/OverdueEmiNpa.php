...
@section('title', 'Overdue & NPA EMI Report')
...
<style>
    ...
    @media (max-width: 767.98px) {
        .excel-table th:not(:nth-child(1)):not(:nth-child(2)):not(:nth-child(5)),
        .excel-table td:not(:nth-child(1)):not(:nth-child(2)):not(:nth-child(5)) {
            display: none;
        }
...
@else
    <main class="col-lg-10 col-md-9 ms-sm-auto px-md-4 py-4 main-content">
        <!-- Header with Sidebar Toggle, Title, Search Bar, and Theme Toggle -->
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
            <div class="d-flex align-items-center gap-2">
                ...
                <h2 class="h3 mb-0 fw-bold">Overdue & NPA EMI Report</h2>
            </div>
            <div class="d-flex align-items-center gap-2">
        ...
        @else
            <div class="excel-table-container mb-4">
                <table class="excel-table" role="grid" aria-describedby="overdueEmiReportTable">
                    <thead>
                        <tr>
             
                            <th scope="col" class="bg-branch">Branch Name</th>
                            <th scope="col">Employee Name</th>
                            <th scope="col">Collection Assign Employee</th>
                            <th scope="col">User Name</th>
                            <th scope="col">Mobile Number</th>
                            <th scope="col">Email ID</th>
                            <th scope="col">Address</th>
                            <th scope="col" class="bg-loan">Loan Amount</th>
                            <th scope="col">Due EMI</th>
                            <th scope="col" class="bg-loan">EMI Amount</th>
                            <th scope="col" class="bg-loan">Half Paid Amount</th>
                            <th scope="col" class="bg-loan">Penalty</th>
                            <th scope="col" class="bg-loan">Total Overdue</th>
                            <th scope="col">Last Due EMI Date</th>
                            <th scope="col">EMI Update</th>
                    ...
                    <tbody>
                        @foreach ($reports as $report)
                            <tr>
                                <td>{{ $report->loan_id ?? 'N/A' }}</td>
                                <td class="branch-name bg-branch">{{ $report->branch_name ?? 'N/A' }}</td>
                                <td class="employee-name">{{ $report->employee_name ?? 'N/A' }}</td>
                                <td class="employee-name">{{ $report->collection_assign_employee ?? $report->employee_name ?? 'N/A' }}</td>
                                <td>{{ $report->user_name ?? 'N/A' }}</td>
                                <td>{{ $report->mobile_number ?? 'N/A' }}</td>
                                <td class="email">{{ $report->email_id ?? 'N/A' }}</td>
                                <td>{{ $report->address ?? 'N/A' }}</td>
                                <td class="bg-loan">{{ is_numeric($report->loan_amount) ? '₹' . number_format(round($report->loan_amount), 2, '.', ',') : 'N/A' }}</td>
                                <td>{{ $report->due_emi ?? 'N/A' }}</td>
                                <td class="bg-loan">{{ is_numeric($report->emi_amount) ? '₹' . number_format(round($report->emi_amount), 2, '.', ',') : 'N/A' }}</td>
                                <td class="bg-loan">{{ is_numeric($report->half_paid_amount) ? '₹' . number_format(round($report->half_paid_amount), 2, '.', ',') : 'N/A' }}</td>
                                <td class="bg-loan">{{ is_numeric($report->penalty) ? '₹' . number_format(round($report->penalty), 2, '.', ',') : 'N/A' }}</td>
                                <td class="bg-loan">{{ is_numeric($report->total_overdue) ? '₹' . number_format(round($report->total_overdue), 2, '.', ',') : 'N/A' }}</td>
                                <td>{{ $report->last_due_emi_date ? \Carbon\Carbon::parse($report->last_due_emi_date)->format('d/m/Y') : 'N/A' }}</td>
                                <td>{{ $report->emi_update ?? 'N/A' }}</td>
                                <td class="{{ $report->status == 'paid' ? 'status-approved' : 'status-closed' }}">{{ $report->status ?? 'N/A' }}</td>
                                <td>
                                    <a href="{{ route('emi.result', ['id' => $report->loan_id]) }}" class="action-btn" title="View Details" aria-label="View loan details for {{ $report->loan_id ?? 'N/A' }}">
