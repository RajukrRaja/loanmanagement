...
@section('title', 'Half Payment Loan Report')
    ...
    <main class="col-lg-10 col-md-9 ms-sm-auto px-md-4 py-4 main-content">
        <div class="header-section">
            <div class="d-flex align-items-center gap-3">
                ...
                <h1>Half Payment Loan Report</h1>
            </div>
            <div class="d-flex align-items-center gap-3">
        ...
        @if ($loans->isEmpty())
            <div class="alert alert-info text-center" role="alert">
                No loans with partial payments found.
            </div>
        @else
            <div class="excel-table-container">
                <table class="excel-table" id="loanTable" role="grid" aria-describedby="halfPaymentLoansTable">
                    <thead>
                        <tr>
                            <th>Sr. No.</th>
                            <th class="bg-branch">Branch Name</th>
                            <th>Employee Name</th>
                            <th style="white-space: nowrap;">User Name</th>
                            <th>Mobile Number</th>
                            <th class="bg-email">Email ID</th>
                            <th class="bg-disbursed">Disbursement Date</th>
                            <th class="bg-loan">Loan Amount</th>
                            <th>Partial Paid</th>
                            <th class="bg-unpaid">Remaining Principal</th>
                            <th>Rest Amount</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($loans as $index => $loan)
                            <tr>
                                ...
                                <td class="bg-loan">
                                    {{ is_numeric($loan->approved_loan_amount) ? '₹' . number_format(round($loan->approved_loan_amount)) : 'N/A' }}
                                </td>
                                <td>
                                    {{ is_numeric($loan->total_partial_paid) ? '₹' . number_format(round($loan->total_partial_paid)) : '₹0' }}
                                </td>
                                <td class="bg-unpaid">
                                    {{ is_numeric($loan->unpaid_principal) ? '₹' . number_format(round($loan->unpaid_principal)) : '₹0' }}
                                </td>
                                <td>
                                    {{ is_numeric($loan->total_rest_amount) ? '₹' . number_format(round($loan->total_rest_amount)) : '₹0' }}
                                </td>
                                <td>
                        ...
                        @empty
                            <tr>
                                <td colspan="12" class="text-center">No loans with partial payments found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    if (table) {
        ...
        function sortTable(key, ascending) {
            ...
            rows.sort((a, b) => {
                ...
                if (key === 'index') {
                ...
                } else if (key === 'approved_loan_amount' || key === 'unpaid_principal' || key === 'total_partial_paid') {
                    ...
                    aValue = parseFloat(a.cells[columnIndex].textContent.replace('%', '')) || 0;
                    bValue = parseFloat(b.cells[columnIndex].textContent.replace('%', '')) || 0;
        ...
        function getColumnIndex(key) {
            const headers = [
                ...
                'email', 'disbursement_date', 'approved_loan_amount', 'total_partial_paid', 'unpaid_principal', 'payment_progress'
            ];
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                link.setAttribute('download', `half_payment_loan_report_${new Date().toISOString().split('T')[0]}.csv`);
