...
@section('title', 'Closure Loan Report')
...
<style>
    ...
    .excel-table th[data-sort="closure_amount"],
    ...
    [data-theme="dark"] .bg-closure-amount {
    ...
    <main class="col-lg-10 col-md-9 ms-sm-auto px-md-4 py-4 main-content">
        <div class="header-section">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h1>Closure Loan Report</h1>
                    <h5 class="text-muted">
                        Total Foreclosed Loans: {{ $loans->total() }}
                    </h5>
                </div>
            </div>
            <div class="d-flex align-items-center gap-3">
        ...
        @else
            <div class="excel-table-container">
                <table class="excel-table" id="loanTable" role="grid" aria-describedby="closureLoansTable">
                    <thead>
                        <tr>
                            <th data-sort="index">Sr. No.</th>
                            <th class="bg-branch" data-sort="branch_name">Branch Name</th>
                            <th data-sort="employee_name">Employee Name</th>
                           
                          <th data-sort="user_name" class="sort-icon" style="white-space: nowrap;">User Name</th>

                            <th data-sort="mobile_no">Mobile Number</th>
                            <th class="bg-email" data-sort="email">Email ID</th>
                            <th class="bg-disbursed" data-sort="disbursement_date">Disbursement Date</th>
                            <th class="bg-loan" data-sort="approved_loan_amount">Loan Amount</th>
                            <th class="bg-closure-amount" data-sort="closure_amount">Closure Amount</th>
                            <th data-sort="closure_date">Closure Date</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($loans as $index => $loan)
                            <tr>
                                ...
                                <td class="bg-branch">{{ $loan->branch_name ?? 'N/A' }}</td>
      <td>{{ $loan->employee_name ?? 'N/A' }}</td>
                               
                                <td>
                                    <a href="{{ route('employee.viewLeadDetails', ['id' => $loan->lead_id]) }}" class="action-link" title="View Lead Details">
                                        {{ trim(($loan->first_name ?? '') . ' ' . ($loan->middle_name ?? '') . ' ' . ($loan->last_name ?? '')) }} ({{ $loan->loan_id ?? 'N/A' }})
                                    </a>
                                </td>
                                <td>{{ $loan->mobile_no ?? 'N/A' }}</td>
                                <td class="bg-email">{{ $loan->email ?? 'N/A' }}</td>
                                <td class="bg-disbursed">
                                    {{ $loan->disbursement_date ? \Carbon\Carbon::parse($loan->disbursement_date)->format('d/m/Y H:i') : 'N/A' }}
                                </td>
                                <td class="bg-loan">
                                    {{ $loan->approved_loan_amount && is_numeric($loan->approved_loan_amount) ? '₹' . number_format(round($loan->approved_loan_amount)) : 'N/A' }}
                                </td>
                                <td class="bg-closure-amount">
                                    {{ $loan->closure_amount && is_numeric($loan->closure_amount) ? '₹' . number_format(round($loan->closure_amount)) : 'N/A' }}
                                </td>
                                <td>
                                    {{ $loan->closure_date ? \Carbon\Carbon::parse($loan->closure_date)->format('d/m/Y H:i') : 'N/A' }}
                                </td>
                                <td>
        ...
        if (table) {
            ...
            function sortTable(key, ascending) {
                ...
                rows.sort((a, b) => {
                    ...
                    } else if (key === 'approved_loan_amount' || key === 'closure_amount') {
                    ...
                    } else if (key === 'disbursement_date' || key === 'closure_date') {
                        aValue = a.cells[columnIndex].textContent === 'N/A' ? new Date(0) : new Date(a.cells[columnIndex].textContent.split(' ')[0].split('/').reverse().join('-') + ' ' + a.cells[columnIndex].textContent.split(' ')[1]);
                        bValue = b.cells[columnIndex].textContent === 'N/A' ? new Date(0) : new Date(b.cells[columnIndex].textContent.split(' ')[0].split('/').reverse().join('-') + ' ' + b.cells[columnIndex].textContent.split(' ')[1]);
            ...
            function getColumnIndex(key) {
                const headers = [
                    ...
                    'email', 'disbursement_date', 'approved_loan_amount', 'closure_amount', 'closure_date'
                ];
        ...
        if (exportBtn) {
            exportBtn.addEventListener('click', () => {
                ...
                setTimeout(() => {
                    ...
                    link.setAttribute('download', `closure_loan_report_${new Date().toISOString().split('T')[0]}.csv`);
