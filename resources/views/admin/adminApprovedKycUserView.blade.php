...
@section('title', 'Approved / Disbursed KYC Users')
...
<style>
    ...
    /* Branch column - Sky blue */
.excel-table td.bg-branch {
    color: #0066cc;
}

.excel-table td.bg-status {
    color: green; /* or #008000 for dark green */
    font-weight: bold;
}


/* Loan Approved Amount column - Dark Blue */
.excel-table td.bg-loan {
    color: #003366;
}

/* Email column - Blue text */
...
color: #ffffff;
    ...
    /* Override background colors for table body cells */
.excel-table td.bg-branch,
.excel-table td.bg-loan {
    background-color: transparent !important;
}

/* Override dark mode background colors for table body cells */
...
[data-theme="dark"] .excel-table td.bg-loan {
    ...
    @media (max-width: 767.98px) {
        ...
        .excel-table th:not(:nth-child(1)):not(:nth-child(2)):not(:nth-child(4)),
        .excel-table td:not(:nth-child(1)):not(:nth-child(2)):not(:nth-child(4)) {
            display: none;
        }
    }
...
@else
    <main class="col-lg-10 col-md-9 ms-sm-auto px-md-4 py-4 main-content">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
            <div class="d-flex align-items-center gap-2">
                ...
                <h2 class="h3 mb-0 fw-bold">Approved / Disbursed KYC Users</h2>
            </div>
            <div class="d-flex align-items-center gap-2">
                <form action="{{ route('admin.adminApprovedKycUserView') }}" method="GET" class="d-flex align-items-center gap-2">
                    @csrf
                    <input type="text" class="form-control search-bar" id="searchInput" name="search" placeholder="Search leads..." value="{{ request()->query('search', '') }}" aria-label="Search leads">
                    <button type="submit" class="btn btn-primary" data-bs-toggle="tooltip" title="Search" aria-label="Search"><i class="fas fa-search"></i></button>
                </form>
        ...
        @elseif ($leads->isEmpty())
            <div class="alert alert-info text-center" role="alert">
                No approved/disbursed KYC users found.
            </div>
        @else
            <div class="excel-table-container">
                <table class="excel-table" role="grid" aria-describedby="kycUsersTable">
                    <thead>
                       <tr>
    <th scope="col">User ID</th>
    <th scope="col" class="bg-branch">Branch Name (Branch ID)</th>
    <th scope="col">Employee Name (Employee ID)</th>
    <th scope="col" style="white-space: nowrap;">Full Name</th>
    <th scope="col">Email</th>
    <th scope="col">Mobile No</th>
    <th scope="col" class="bg-status">Status</th>
    ...
    <th scope="col">Actions</th>
                    ...
                    <tbody>
                        @foreach ($leads as $lead)
                            <tr>
                                ...
                                <td class="branch-name bg-branch">{{ $lead->branch_name && $lead->branch_id ? ($lead->branch_name . ' (' . $lead->branch_id . ')') : 'N/A' }}</td>
                                <td class="employee-name">{{ $lead->employee_name && $lead->employee_id ? ($lead->employee_name . ' (' . $lead->employee_id . ')') : 'N/A' }}</td>
                                <td>
                                ...
                                <td class="{{ $lead->kyc_status === 'Approved' ? 'status-approved' : ($lead->kyc_status === 'Disburse' ? 'status-approved' : 'status-closed') }}">
                                    {{ $lead->kyc_status ?? 'N/A' }}
                                </td>
                                <td class="bg-loan">{{ is_numeric($lead->approved_loan_amount) ? '₹' . number_format($lead->approved_loan_amount, 0, '.', ',') : 'N/A' }}</td>
                                <td>{{ $lead->branch_recommendation ?? 'N/A' }}</td>
                                <td>{{ $lead->created_at ? \Carbon\Carbon::parse($lead->created_at)->format('d/m/Y H:i') : 'N/A' }}</td>
                                <td>
                                    <a href="{{ route('admin.adminApprovedKycUserEmiCalculation', ['lead_id' => $lead->lead_id]) }}" class="action-btn" data-bs-toggle="tooltip" title="View EMI Details" aria-label="View EMI Details">
            ...
            <div class="d-flex justify-content-center">
                {{ $leads->appends(['search' => request()->query('search')])->links('pagination::bootstrap-5') }}
            </div>
        @endif
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (sidebarToggle && sidebar) {
        sidebarToggle.addEventListener('click', () => {
            ...
            if (main) {
                main.style.marginLeft = sidebar.classList.contains('show') ? '240px' : '0';
            }
        });
    }
    ...
    document.querySelectorAll('.sidebar .nav-link').forEach(link => {
        ...
        if (link.getAttribute('href') === '{{ route('admin.adminApprovedKycUserView') }}') {
