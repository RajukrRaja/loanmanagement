            ...
            @else
                ...
                @if($permissions['can_read'])
                    ...
                    <div class="table-container">
                        <table class="user-table" id="userTable" role="grid" aria-describedby="subAdminUsersTable">
                            <thead>
                                <tr>
                                    <th data-sort="s_no" class="sort-icon">Sr.no</th>
                            ...
                            <tbody id="userTableBody">
                                @forelse ($users as $index => $user)
                                    <tr data-user-id="{{ $user->id }}" data-role-id="{{ $user->role_id ?? '' }}" data-branch-id="{{ $user->branch_id ?? '' }}">
                                        <td>{{ $index + 1 }}</td>
                                        <td class="employee-name">{{ $user->name }}</td>
                                        <td class="email">{{ $user->email }}</td>
                                        <td class="status-kyc bg-status">{{ $user->role->name ?? 'Sub Admin' }}</td>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (exportBtn) {
            exportBtn.addEventListener('click', () => {
                ...
                setTimeout(() => {
                    ...
                    link.setAttribute('download', `sub_admin_users_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        document.querySelectorAll('.edit-user-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                form.action = window.routes.updateUser.replace(':subadmin', userId);
        ...
        document.querySelectorAll('.delete-user-btn').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (confirm('Are you sure you want to delete this user?')) {
                    ...
                    btn.disabled = true;
                    try {
                        const response = await fetch(window.routes.deleteUser.replace(':subadmin', btn.dataset.userId), {
                            method: 'DELETE',
                            headers: {
                                'Accept': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        document.querySelectorAll('.sidebar .nav-link').forEach(link => {
            ...
            if (link.getAttribute('href') === '{{ route('superadmin.SubadminUserView') }}') {
