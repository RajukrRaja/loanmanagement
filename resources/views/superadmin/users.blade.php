...
color: #ffffff;
    ...
    [data-theme="dark"] .user-table .branch-name {
    ...
    @media (max-width: 991.98px) {
        ...
        .user-table th:nth-child(n+6),
        .user-table td:nth-child(n+6) {
            display: none; /* Hide created_at, updated_at on smaller screens */
        }
    }
    ...
    @else
        <main class="col-lg-10 col-md-9 ms-sm-auto px-md-4 py-4 main-content">
            <div class="header-section">
                ...
                <div class="d-flex align-items-center gap-3">
                    <input type="text" class="form-control search-bar" placeholder="Search users..." id="userSearch" aria-label="Search users">
            ...
            <div class="table-container">
                <table class="user-table" id="userTable" role="grid" aria-describedby="manageUsersTable">
                    <thead>
                        <tr>
                            <th data-sort="id" class="sort-icon">ID</th>
                            ...
                            <th data-sort="created_at" class="sort-icon bg-date">Created At</th>
                            <th data-sort="updated_at" class="sort-icon bg-date">Updated At</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody id="userTableBody">
                        @forelse ($users as $user)
                            <tr data-user-id="{{ $user->id }}" data-role="{{ $user->role }}" data-branch-id="{{ $user->branch_id ?? '' }}">
                                <td>{{ $user->id }}</td>
                                <td class="employee-name">{{ $user->name }}</td>
                                <td class="email">{{ $user->email }}</td>
                                <td class="status-kyc bg-status">{{ $user->role->name ?? 'N/A' }}</td>
                                <td class="status-current bg-branch">{{ $user->branch ? $user->branch->branch_name : 'N/A' }}</td>
                                <td class="bg-date">{{ $user->created_at ? \Carbon\Carbon::parse($user->created_at)->format('d/m/Y H:i') : 'N/A' }}</td>
                                <td class="bg-date">{{ $user->updated_at ? \Carbon\Carbon::parse($user->updated_at)->format('d/m/Y H:i') : 'N/A' }}</td>
                                <td>
                                    <a href="" class="btn btn-sm btn-outline-secondary view-user-btn" data-user-id="{{ $user->id }}" data-bs-toggle="tooltip" title="View"><i class="fas fa-eye"></i></a>
                                    <button class="btn btn-sm btn-primary edit-user-btn" data-user-id="{{ $user->id }}" data-bs-toggle="modal" data-bs-target="#editUserModal" data-bs-toggle="tooltip" title="Edit"><i class="fas fa-edit"></i></button>
                                    <button class="btn btn-sm btn-outline-danger delete-user-btn" data-user-id="{{ $user->id }}" data-bs-toggle="tooltip" title="Delete"><i class="fas fa-trash"></i></button>
                                </td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="pagination">
                {{ $users instanceof \Illuminate\Pagination\LengthAwarePaginator ? $users->links('pagination::bootstrap-5') : '' }}
            </div>
                            ...
                            <form id="newUserForm" action="{{ route('superadmin.users.store') }}" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="role" class="form-select" required>
                                        ...
                                        <option value="Branch Login">Branch Login</option>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (table) {
            ...
            function getColumnIndex(key) {
                const headers = ['id', 'name', 'email', 'role', 'branch', 'created_at', 'updated_at'];
        ...
        document.querySelectorAll('.edit-user-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                form.querySelector('[name="email"]').value = row.cells[2].textContent;
                form.querySelector('[name="password"]').value = '';
                form.querySelector('[name="role"]').value = row.cells[3].textContent || '';
        ...
        document.querySelectorAll('.sidebar .nav-link').forEach(link => {
            ...
            if (link.getAttribute('href') === '{{ route('superadmin.manage_user') }}') {
