...
<style>
    ...
    .user-table .employee-name {
        color: #cc6600; /* Orange text color */
    }

    .user-table .email {
        color: #ff0000 !important; /* Red text for email column */
        font-weight: 600;
    }

    .user-table .status-kyc {
        color: #28a745 !important; /* Green for Role (proxy for KYC Status) */
        font-weight: 500;
        text-transform: uppercase;
        letter-spacing: 0.5px;
    }

    .user-table .status-current {
        color: #dc3545 !important; /* Red for Branch (proxy for Current Status) */
        font-weight: 500;
        text-transform: uppercase;
        letter-spacing: 0.5px;
    }
        ...
        <main class="col-lg-10 col-md-9 ms-sm-auto px-md-4 py-4 main-content">
                        ...
                        <tbody id="userTableBody">
                            @forelse ($users as $index => $user)
                                 <tr data-user-id="{{ $user->id }}" data-role-id="{{ $user->role->id ?? '' }}" data-branch-id="{{ $user->branch_id ?? '' }}">
                                    ...
                                    <td class="status-kyc bg-status">{{ $user->role ?? 'Sub Branch Admin' }}</td>
            ...
            @else
                <div class="alert alert-warning text-center">
                    You do not have permission to view users. Role ID: {{ Auth::user()->role_id }}, Dropdown Item ID: 5. Contact your administrator.
                </div>
            @endif
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.addEventListener('click', (e) => {
            ...
            const isClickInside = Array.from(toggles).some(toggle => toggle.contains(e.target)) || 
                                 Array.from(dropdowns).some(dropdown => dropdown.contains(e.target));
            if (!isClickInside) {
        ...
        if (exportBtn) {
            exportBtn.addEventListener('click', () => {
                ...
                setTimeout(() => {
                    ...
                    link.setAttribute('download', `sub_branch_admin_users_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        document.querySelectorAll('.edit-user-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                form.action = window.routes.updateUser.replace(':subbranchadmin', userId);
        ...
        document.querySelectorAll('.delete-user-btn').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (confirm('Are you sure you want to delete this user?')) {
                    ...
                    btn.disabled = true;
                    try {
                        const response = await fetch(window.routes.deleteUser.replace(':subbranchadmin', btn.dataset.userId), {
                            method: 'DELETE',
                            headers: {
                                'Accept': 'application/json',
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
