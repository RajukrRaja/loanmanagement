...
@section('title', 'Team Manager Users')
        ...
        <main class="col-lg-10 col-md-9 ms-sm-auto px-md-4 py-4 main-content">
            ...
            @else
                ...
                @if($permissions['can_read'])
                    <!-- Team Manager Users Table -->
                    <div class="table-container">
                        <table class="user-table" id="userTable" role="grid" aria-describedby="teamManagerUsersTable">
                            <thead>
                                <tr>
                            ...
                            <tbody id="userTableBody">
                                @forelse ($users as $index => $user)
                                    <tr data-user-id="{{ $user->id }}" data-role-id="{{ $user->role->id ?? '' }}" data-branch-id="{{ $user->branch_id ?? '' }}">
                                        ...
                                        <td class="status-kyc bg-status">{{ $user->role->name ?? 'Team Manager' }}</td>
        ...
        if (exportBtn) {
            exportBtn.addEventListener('click', () => {
                ...
                setTimeout(() => {
                    ...
                    link.setAttribute('download', `team_manager_users_${new Date().toISOString().split('T')[0]}.csv`);
